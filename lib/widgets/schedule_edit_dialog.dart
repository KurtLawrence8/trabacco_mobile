...
class _ScheduleEditDialogState extends State<ScheduleEditDialog> {
  ...
  void _loadCurrentScheduleData() {
    // Load current unit and budget
    if (widget.schedule.laborers != null &&
        widget.schedule.laborers!.isNotEmpty) {
      final laborer = widget.schedule.laborers![0];
    ...
    // Load currently assigned laborers
    if (widget.schedule.laborers != null &&
        widget.schedule.laborers!.isNotEmpty) {
      for (var laborerData in widget.schedule.laborers!) {
        if (laborerData is Map<String, dynamic>) {
          final laborer = laborerData['laborer'];
          if (laborer is Map<String, dynamic>) {
            final firstName = laborer['first_name']?.toString() ?? '';
            final middleName = laborer['middle_name']?.toString() ?? '';
            final lastName = laborer['last_name']?.toString() ?? '';
            final phoneNumber = laborer['phone_number']?.toString();
            final id = laborer['id'] as int?;
            final createdAt = laborer['created_at']?.toString();
            final updatedAt = laborer['updated_at']?.toString();

            if (id != null) {
              // This is an existing laborer
              final existingLaborer = Laborer(
                id: id,
                firstName: firstName,
              ...
              _selectedLaborers.add(existingLaborer);
            }
          }
        }
      }
  ...
  Future<void> _updateSchedule() async {
    ...
    try {
      ...
      _showSuccessSnackBar(
          'Schedule updated with $totalAssigned laborer(s) assigned');

      Navigator.of(context).pop();
    } catch (e) {
      _showErrorSnackBar('Failed to update schedule: $e');
    } finally {
      setState(() => _isLoading = false);
    }
  }
  ...
  Widget _buildScheduleDetailsFields() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text(
