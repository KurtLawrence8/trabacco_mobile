...
class _PlantingFormScreenState extends State<PlantingFormScreen> {
  ...
  Future<void> _saveDraft() async {
    try {
      ...
      final draft = {
        ...
        'plantsPlanted': _plantsPlantedController.text,
        'selectedLaborerIds': _selectedLaborers.map((l) => l.id).toList(),
      };
      await prefs.setString('planting_form_draft', jsonEncode(draft));
    } catch (e) {
  ...
  Future<void> _loadDraft() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final draftString = prefs.getString('planting_form_draft');
      if (draftString != null && mounted) {
        final draft = jsonDecode(draftString) as Map<String, dynamic>;

        setState(() {
          _plantsPlantedController.text = draft['plantsPlanted'] ?? '';

          // Load selected farm if available
          if (draft['selectedFarmId'] != null) {
            // Will be set after farms are loaded
          }

          // Load selected laborers if available - will be handled after laborers are loaded
        });
      }
    } catch (e) {
  ...
  Future<void> _clearDraft() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.remove('planting_form_draft');
    } catch (e) {
      // Silently fail
    }
  }
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    _buildInfoRowNoIcon(
                        'Plants Planted', _plantsPlantedController.text),
                    const SizedBox(height: 16),
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 8),
                    Text(
                      'You have already submitted for this farm',
                      style: TextStyle(
                        color: Colors.white.withOpacity(0.8),
                        fontSize: 14,
                      ),
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    const Text(
                      'It appears you\'ve already submitted a planting report for this farm. Please check your records or contact support if you need to make changes.',
                      style: TextStyle(
                        color: Color(0xFF2C3E50),
                        fontSize: 16,
                        height: 1.5,
  ...
  Future<void> _submitPlantingReport() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // Check if it's a duplicate submission error in exception
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        Column(
                          children: [
                            ...
                            Semantics(
                              ...
                              hint: _selectedFarm != null
                                  ? 'Currently selected: ${_selectedFarm!.farmAddress}${_selectedFarm!.name != null && _selectedFarm!.name!.isNotEmpty ? " - ${_selectedFarm!.name}" : ""}'
                                  : 'Tap to select a farm',
                              button: true,
                    ...
                    // Combined Laborers and Plants Planted Section
                    Container(
                      padding: const EdgeInsets.all(16),
                      decoration: BoxDecoration(
                        color: Colors.white,
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              Expanded(
                                child: Semantics(
                                  label: 'Add laborers to the planting report',
                                  button: true,
                                  child: OutlinedButton(
                          ...
                          Semantics(
                            ...
                            hint: 'Enter the total number of plants planted',
                            textField: true,
                            child: TextFormField(
                              controller: _plantsPlantedController,
                              focusNode: _plantsPlantedFocusNode,
                              decoration: InputDecoration(
                                hintText: 'Enter number of plants planted',
                    ...
                    // Submit Button
                    Semantics(
                      label: 'Submit planting report',
                      hint:
                          'Tap to submit the planting report with current form data',
                      button: true,
                      child: ElevatedButton(
  ...
  void dispose() {
    ...
    _plantsPlantedController.removeListener(_saveDraft);
    ...
    _plantsPlantedFocusNode.dispose();
    super.dispose();
  }
}
